...
## Objective
Automatically scan the `00_inbox` folder inside the `TECH-BRAIN` directory on Google Drive and process any files found within it.
...
1.  **Scan `00_inbox`**: Find the `00_inbox` folder inside the `TECH-BRAIN` root directory. List all non-trashed files inside it.
